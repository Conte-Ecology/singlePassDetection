...
library(reshape2)
spring.t.long <- annual.site.climate %>%
  filter(season == "Spring") %>%
  filter(!is.na(site)) %>%
  filter(year >= 1997 & year <= 2010)

spring.t.long <- spring.t.long[c("site", "year", "mean.tmean")]

spring.t.wide <- reshape(spring.t.long, idvar = c("site"), timevar = "year", direction = "wide")

spring.temp <- spring.t.wide[ , 2:15]

# Bundle data
Dat <- list(nSites=nSites, nYears=nYears, nAges=nAges, 
            y=y, elev=elev, Jdate=data.frame(rep(0, times=72), Jdate), width=data.frame(rep(0, 72), width), 
...
export.list <- list("Dat", "Init", "SNew", "nSites", "nYears", "nAges", "nPasses", "model", "n.burn")
out.yo.coda <- runJagsModel(model = "src/landscape-DM-model.R", data=Dat, inits=Init, export.list, n.chains = 3, n.adapt=100, params, n.iter = 1000, thin = 3, jags.out = FALSE)

CL <- makeCluster(3)
  clusterExport(cl=CL, list("Dat", "Init", "SNew", "nSites", "nYears", "nAges", "nPasses", "model", "n.burn", "ParsStage"), envir = environment())
  clusterSetRNGStream(cl=CL, iseed = 2345642)
  
  system.time(out <- clusterEvalQ(CL, {
    library(rjags)
    load.module('glm')
    jm <- jags.model(paste0("src/landscape-DM-model.R"), data=Dat, inits=Init, n.adapt=10000, n.chains=1)
    fm <- coda.samples(jm, params = ParsStage, n.iter = 20000, thin = 50)
    return(as.mcmc(fm))
  }))
...
# Sequential
StageBurnin <- jags.model(paste0("src/landscape-DM-model.R"), 
                          Dat, Init, n.chains=3, n.adapt=3000)       
...
## run
Niter=3000
out1 <- coda.samples(StageBurnin, ParsStage, n.iter=Niter, thin=10)
summary(out1)
plot(mcmc.list(out1[ , c("beta", "omega.b")]))
...
ParsStage <- c("omega.mean","omega.b","gamma.b","lambda",
               "Ntotal","p.mean","p.b","alpha","beta","N","p")
## run
out2 <- jags.samples(StageBurnin, ParsStage, n.iter=Niter, thin=10)
str(out2)
```

## Extract Abundance and Detection Estimates
```{r}
N.est <- N.uci <- N.lci <- p.est <- p.uci <- p.lci <- array(NA, dim=c(nSites,nYears,nAges))
for(i in 1:nSites){
  for(t in 1:nYears){
    for(a in 1:nAges){
      N.est[i,t,a] <- median(out2$N[i,t,a,1:200,1:3])
      N.lci[i,t,a] <- quantile(out2$N[i,t,a,1:200,1:3], probs = c(0.025))
      N.uci[i,t,a] <- quantile(out2$N[i,t,a,1:200,1:3], probs = c(0.975))
      p.est[i,t,a] <- median(out2$p[i,t,a,1:200,1:3])
      p.lci[i,t,a] <- quantile(out2$p[i,t,a,1:200,1:3], probs = c(0.025))
      p.uci[i,t,a] <- quantile(out2$p[i,t,a,1:200,1:3], probs = c(0.975))
    }
  }
}

shapeLong <- function(data, age = "yoy") {
  if(age == "yoy") {
    A <- 1
  } else if(age == "adult") {
    A <- 2
  } else {
    stop("age must be 'yoy' or 'adult'")
  }
  foo <- data.frame(data[ , , A])
  foo$site <- paste0("Site", row.names(foo))
  colnames(foo) <- c(1996:2010, "site")
  bar <- reshape::melt(foo, id = "site")
  names(bar) <- c("site", "year", paste0("N.",age))
  cash <- mutate(bar, site.year = paste0(site, "-", year))
  long <- cash[ , c("site.year", paste0("N.",age))]
  return(long)
}

shapeLong(N.est, age = "yoy") # should be ok
shapeLong(N.est, age = "adult") # should be ok
shapeLong(N.est, age = "YOY") # should fail

Nhat.dm.yoy <- shapeLong(N.est, age = "yoy")
lci.dm.yoy <- shapeLong(N.lci, age = "yoy")
uci.dm.yoy <- shapeLong(N.uci, age = "yoy")

Nhat.dm.adult <- shapeLong(N.est, age = "adult")
lci.dm.adult <- shapeLong(N.lci, age = "adult")
uci.dm.adult <- shapeLong(N.uci, age = "adult")

N.dm.yoy <- data.frame(site.year = Nhat.dm.yoy$site.year, N.dm.yoy = Nhat.dm.yoy$N.yoy, lci.dm.yoy = lci.dm.yoy$N.yoy, uci.dm.yoy = uci.dm.yoy$N.yoy)

N.dm.adult <- data.frame(site.year = Nhat.dm.adult$site.year, N.dm.adult = Nhat.dm.adult$N.adult, lci.dm.adult = lci.dm.adult$N.adult, uci.dm.adult = uci.dm.adult$N.adult)

save(N.dm.yoy, N.dm.adult, file = "data/N-dail-madsen.RData")
...
recruit.coefs <- NULL
for(i in 1:5) {
  foo <- quantile(out2$gamma.b[i, , ], probs = c(0.5, 0.025, 0.975))
  recruit.coefs <- rbind(recruit.coefs, foo)
  }
row.names(recruit.coefs) <- c("fall.flow", "winter.flow", "spring.flow", "spring.temp", "elev")
recruit.coefs

survival.coefs.adult <- NULL
for(i in 1:5) {
  foo <- quantile(out2$omega.b[i, 2, , ], probs = c(0.5, 0.025, 0.975))
  survival.coefs.adult <- rbind(survival.coefs.adult, foo)
  }
row.names(survival.coefs.adult) <- c("fall.flow", "winter.flow", "spring.flow", "spring.temp", "elev")
survival.coefs.adult

survival.coefs.yoy <- NULL
for(i in 1:5) {
  foo <- quantile(out2$omega.b[i, 1, , ], probs = c(0.5, 0.025, 0.975))
  survival.coefs.yoy <- rbind(survival.coefs.yoy, foo)
  }
row.names(survival.coefs.yoy) <- c("fall.flow", "winter.flow", "spring.flow", "spring.temp", "elev")
survival.coefs.yoy

# detection
detect.coefs <- NULL
for(i in 1:2) {
  foo <- quantile(out2$p.b[i, , ], probs = c(0.5, 0.025, 0.975))
  detect.coefs <- rbind(detect.coefs, foo)
  }
row.names(detect.coefs) <- c("day", "width")
detect.coefs

detect.coefs.mean <- NULL
for(i in 1:2) {
  foo <- quantile(out2$p.mu[i, , ], probs = c(0.5, 0.025, 0.975))
  detect.coefs.mean <- rbind(detect.coefs.mean, foo)
  }
row.names(detect.coefs.mean) <- c("yoy", "adult")
detect.coefs.mean
