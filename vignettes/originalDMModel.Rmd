...
out.yo.coda <- runJagsModel(model = "src/Yoichiro-DM-Model.R", data=Dat, inits=Init, export.list, n.chains = 3, n.burn=1000, params, n.iter = 1000, thin = 3, jags.out = FALSE)
...
CL <- makeCluster(3)
  ...
  system.time(out <- clusterEvalQ(CL, {
    library(rjags)
    load.module('glm')
    jm <- jags.model(paste0("src/Yoichiro-DM-Model.R"), data=Dat, inits=Init, n.adapt=10000, n.chains=1)
    fm <- coda.samples(jm, params, n.iter = 20000, thin = 50)
    return(as.mcmc(fm))
  }))
...
# Sequential
StageBurnin <- jags.model(paste0("src/Yoichiro-DM-Model.R"), 
                          Dat, Init, n.chains=3, n.adapt=100)       
